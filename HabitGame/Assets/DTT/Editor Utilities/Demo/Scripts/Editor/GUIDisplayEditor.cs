...
namespace DTT.Utils.EditorUtilities
{
    ...
    internal class GUIDisplayEditor : Editor
    {
        ...
        {
            _foldout = new AnimatedFoldout(this, true, true);
        }
        ...
        {
            GUIContent content = new("Open Extended Dropdown");
            Rect buttonRect = GUILayoutUtility.GetRect(content, GUI.skin.button);
            if (GUI.Button(buttonRect, content))
            {
                ExtendedDropdownBuilder builder = new("Extended Dropdown", buttonRect, new AdvancedDropdownState());
                ExtendedDropdownItem[] items = new[] { "Monkey", "Giraffe", "Toad" }.Select(animalName => new ExtendedDropdownItem
                {
                    name = animalName
                }).ToArray();
                _ = builder.StartIndent("Animals").AddItems(items).EndIndent();
        ...
        {
            _ = _foldout.OnGUI("Animated Foldout", () =>
            {
                GUILayout.Label("The first label");
                GUILayout.Label("The second label");
            }, DrawHeaderButton);

            static void DrawHeaderButton(Rect foldoutRect)
            {
                float singleLineHeight = EditorGUIUtility.singleLineHeight;
                GUIContent content = new(EditorGUIUtility.IconContent("_Help"));
